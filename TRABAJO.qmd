...
En este trabajo, se busca relacionar la condición de tener HTA o no tener HTA con algunas covariables, tales como el hábito de fumar, realizar actividad física, consumo de alcohol, entre otras. Esto con el objetivo de evaluar el efecto que pueden tener estos factores sobre la probabilidad de ser HTA.

En primer lugar, se observa la relación existente entre la presencia de HTA y la edad, teniendo en cuenta la base de datos del estudio. Para esto se agrupan las personas que presentan HTA y aquellas que no, para mostrar gráficamente la distribución entre hombres y mujeres.

```{r echo=FALSE, message=FALSE, warning=FALSE}
"Grafico de cajas y bigotes para el sexo y presencia de HTA"

ggplot(Base, aes(x= Sexo,  group=HTA)) + 
  ...
  ggtitle("Gráfico 1: Proporción sexo según presencia de HTA")
```

Como se puede observar, entre las personas con HTA, las hombres representan la mayor proporción con un 51.22% del total de personas que presentan esta condición. Mientras que los mujeres representan el 48.78% del total de personas no HTA.Sin embargo, esta herramienta visual no permite definir si verdaderamente existe una relación significativa entre la edad y tener o no HTA, por lo que se realiza una prueba Chi, con el objetivo de evaluar la independencia entre estas variables. La prueba consta de las siguientes hipótesis:

$$\begin{aligned}
H_0&:\mbox{El sexo es independiente en la presencia de la HTA} vs \\
H_1&:\mbox{El sexo es dependiente en la presencia de la HTA}
\end{aligned}
 $$
...
t1.2 <- round(prop.table(table(Base$HTA, Base$Sexo))*100,2)
addmargins(t1.2)
...
kable(tchi,col.names = c("Variables","Estadístico","gdl","P-valor")) %>%
  ...
  add_header_above(c("Tabla 2: Prueba chi para las variables HTA y sexo"=4 ))%>%
  column_spec(2, width = "5em")%>%
  kable_styling(fixed_thead = T)
```

A partir de los resultados de la prueba plasmados en la tabla anterior, se puede afirmar que con una confianza del $95\%$, la hipótesis nula se acepta ya que no existe diferencia significativa para sostener que la presencia de HTA es independiente del sexo de las personas. Por otra parte, otro factor con el que se podría relacionar la presencia de HTA, es con la edad, de manera similar que con el sexo, es posible evaluar si existe independencia entre ambas variables. Para este caso se compara una variable categórica nominal (dicotomica) que se traduce en tener o no HTA y la edad como continua. En este caso, se presenta el siguiente grafico de cajas y bigotes, para comparar cuartiles, entre ellos la mediana para la edad, teniendo en cuenta que los datos se agrupan en tener o no HTA.

A simple vista, la diferencia entre los dos grupos respecto a la edad no parece ser muy grande; De las personas con HTA se encuentra que la persona con más edad, se encuentra entre los 40 años y 50.

Sí tomamos el 25% de las personas sin HTA, son menores a 20 años, y la gran mayoría de las personass con HTA tiene menos de 40 años.

En esta conclusión se pudo deducir que al comparar las edades para ambos grupos, se evidencia que para el grupo de personas con HTA las edades son mayores vs las personas sin dicha patología, y respecto a las medias se distancian por 4, sin embargo las desviaciones para ambos grupos (tabla 3), ambos parecen obtener resultados muy similares.

```{r echo=FALSE, message=FALSE, warning=FALSE}

"Gráfico de Caja y bigotes para la edad, de acuerdo a la presencia de HTA"

bp <- ggplot(Base, aes(x =HTA, y = Edad, fill =HTA)) 
bp + geom_boxplot(show.legend = F) + scale_fill_brewer(palette = "Blues")+
  xlab(NULL)+
  ylab("Edad")+
  ggtitle("Gráfico 2: Caja y bigotes para la edad, de acuerdo a la presencia de HTA")
...
kable(t2,col.names = c("Condición","Casos" ,"Media","Desviación estandar")) %>%
  kable_paper("hover", full_width = F,position="center") %>%
  add_header_above(c("Tabla 3: Relación presencia de HTA y edad"=4 ))%>%
...
$$\begin{aligned}
H_0&: \mbox{media de edad HTA = media de edad no HTA} vs\\
H_1&: \mbox{media de edad HTA} \neq \mbox{media de edad no HTA}
\end{aligned}$$
...
pruebat=t.test(Base$Edad~Base$HTA,var.equal=TRUE)

t3=t(c(
  HTA=round(pruebat$estimate[1],2),
  noHTA=round(pruebat$estimate[2],2),
...
kable(t3,col.names = c("Promedio grupo con HTA", "promedio grupo sin HTA", "Estadístico","gdl","Límite inf","Límite sup","P-valor")) %>%
  kable_paper("hover", full_width = F,position="center") %>%
  add_header_above(c("Tabla 4: Prueba t para la diferencia de medias entre las edades de HTA y no HTA"=7 ))%>%
  column_spec(1:7, width = "5em")%>%
  kable_styling(fixed_thead = T)
```

A partir de los resultados obtenidos y un pvalor mayor que el $5\%$, se puede afirmar que con una confianza del $95\%$, la hipótesis nula se acepta, por lo que hay evidencia estadística para sostener que no hay diferencia entre el promedio de las edades de los dos grupos.

## Modelo de Regresión logístico inicial

Como se mencionó al principio de este trabajo, se busca relacionar la probabilidad de tener HTA frente a ciertas covariables. Es por esto, que se realiza un un modelo de regresión logística entre la variable HTA y las siguientes variables independientes:
...
$log\left [ p(HTA)) \right ]=ln\left [ \frac{P(Tener HTA)}{1-P(Tener HTA)}\right]=$-16.97 - 0.44Fuma+ 0.56 Alcohol + 16.123 *Act. Fisica* +2.22 *Antobesidad* + 0.71 *Bioquímicas*.
...
mod1 <-glm(formula=HTA~Fuma+Alcohol+ `Actividad Fisica` + ANTOBESIDAD+BioquimicAS, family = binomial,data = Base)

summ(mod1, model.info = FALSE, digits =2,)
...
#En el siguiente modelo predictivo escogí el paciente número 2: el presenta en la variable fumar 2, alcohol 2 Activ fisica 1 Antb obesidad 2, Bioquimica 1
...
$$Odds=\frac{P(Tener HTA)}{1-P(Tener HTA)}=e^{logit}$$
